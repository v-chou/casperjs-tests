...
casper.test.begin('Create IP Right SFI Test', function(test) {
...
// We will first complete the project information page.    
    ...
    // casper.then(function(){
      ...
      casper.selectOptionByText('#otherEntity.control-form','Monsanto');
      ...
      this.clickLabel('Enter Required SFI', 'button');
      /*
        Waits for the modal dialog to appear then fill the necessary fields
      */
     
     this.waitUntilVisible('#rightsModal', function(){
        test.assertVisible('h3#rightsModalLabel','SFI Disclosure for Intellectual Property Rights');
        // this.capture('./images/rightsModal.png');
        this.fillSelectors('.sfi-ip-modal', {
          'select[name="ipEntity"]': 'Monsanto',
          'select[name="ipPayment"]': '17',
          'select[name="ipRelationship"]': 'SELF',
          'textarea[name="ipDescription"]': 'Inserted by CasperJS on: ' + mydigits + text2202chars,
          'input[name="ipSfiRelated"]': yesNoOption,
          'textarea[name="ipStatement"]': 'Inserted by CasperJS  on: ' + mydigits + text2202chars
        }, false);
        this.click('button.submit-ip-modal');
        // this.capture('./images/rightsModalFilled.png');
     });
});
